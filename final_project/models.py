...
class FeatureExtraction(nn.Module):
    def __init__(self, input_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        conv = nn.Conv2d(input_nc, ngf, kernel_size=4, stride=2, padding=1)

        model = [conv, nn.ReLU(True), norm_layer(ngf)]
        ...
        for i in range(3):
            ...
            if(2 ** i * ngf < 512):
                ...
                out_ngf = 512
            conv = nn.Conv2d(in_ngf, out_ngf, 4, 2, 1)
            model += [conv, nn.ReLU(True)]
            model += [norm_layer(out_ngf)]

        model += [nn.Conv2d(512, 512, 3, padding = 1), nn.ReLU(True)]
...
class FeatureL2Norm(torch.nn.Module):
    ...
    def forward(self, feature):
        epsilon = 1e-6
        normalized = torch.pow(torch.sum(torch.pow(feature, 2), 1)+ epsilon , 0.5)
        normalized = normalized.unsqueeze(1).expand_as(feature)
        return torch.div(feature, normalized)
...
class TpsGridGen(nn.Module):
    def __init__(self, out_h=256, out_w=192, use_regular_grid=True, reg_factor=0, use_cuda=True):
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            upconv = nn.Conv2d(inner_nc * 2, outer_nc, 3, padding=1, bias=use_bias)
            down = [downconv]
            up = [uprelu, upsample, upconv, upnorm]
            model = down + [submodule] + up
        elif innermost:
            upsample = nn.Upsample(scale_factor=2, mode='bilinear')
            upconv = nn.Conv2d(inner_nc, outer_nc, 3, padding=1, bias=use_bias)
